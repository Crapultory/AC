...
def _isolated_env(tmp_path: Path) -> dict[str, str]:
    """Keep subprocess dotenv loading from importing the checkout's secrets."""
    ...
    # The checkout may have a development .env with A2A enabled.  Seed both
    # keys so the loader's project-env fallback cannot change this test's
    # explicit security mode.
    env["AISOC_A2A_AUTH"] = "false"
    env["A2A_SESSION_TOKEN"] = ""
    return env


@pytest.mark.asyncio
async def test_hermes_aisoc_a2a_e2e_message_roundtrip(tmp_path: Path) -> None:
    port = _free_port()
...
@pytest.mark.asyncio
async def test_hermes_aisoc_a2a_e2e_streaming_roundtrip(tmp_path: Path) -> None:
    port = _free_port()
    env = _isolated_env(tmp_path)
    env["AISOC_A2A_TEST_MODE"] = "echo"
...
@pytest.mark.asyncio
async def test_hermes_aisoc_a2a_e2e_message_roundtrip_with_auth(tmp_path: Path) -> None:
    port = _free_port()
    env = _isolated_env(tmp_path)
    env["AISOC_A2A_TEST_MODE"] = "echo"
