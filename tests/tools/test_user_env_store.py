...
def test_make_user_env_key_encodes_platform_and_components(monkeypatch, tmp_path):
    ...
    assert key == "we.com.user%2F42.Alice.Bob"
...
def test_load_user_env_isolated_by_platform(monkeypatch, tmp_path):
    ...
    _write_users_env(
        tmp_path,
        {
            "slack.u123.alice": {"API_TOKEN": "slack-token"},
            "telegram.u123.alice": {"API_TOKEN": "telegram-token"},
        },
    )
    ...
    assert slack.user_key == "slack.u123.alice"
    assert slack.env == {"API_TOKEN": "slack-token"}
    assert telegram.user_key == "telegram.u123.alice"
    assert telegram.env == {"API_TOKEN": "telegram-token"}


def test_load_user_env_migrates_single_old_username_within_same_platform(monkeypatch, tmp_path):
    monkeypatch.setenv("HERMES_HOME", str(tmp_path))
    ...
    assert loaded.user_key == "slack.u123.new-name"
    assert loaded.env == {"API_TOKEN": "old-token"}

    payload = json.loads(path.read_text(encoding="utf-8"))
    assert "slack.u123.old-name" not in payload
    assert payload["slack.u123.new-name"] == {"API_TOKEN": "old-token"}
