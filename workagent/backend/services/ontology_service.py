...
ONTOLOGY_SUBPROCESS_TIMEOUT = int(os.environ.get("WORKAGENT_ONTOLOGY_SUBPROCESS_TIMEOUT", "300"))
...
# Absolute path to the workagent profile, resolved via the OS uid → home mapping
...
_PROFILE = f"{_real_home()}/.hermes/profiles/workagent"
...
class OntologyService:
    ...
    # citations are surfaced by the workagent-ontology skill's own scripts, not by
    ...
    def _deprecated_build_chat_answer(query: str) -> dict[str, Any]:
        ...
        answer = (
            f"当前 WORKAGENT Ontology 完整度为 {scorecard.get('completeness_score')}/100。"
            + "；".join(summaries)
            + "。回答基于最近一次扫描快照与证据路径。"
        )
    ...
    def build_roadmap() -> dict[str, Any]:
        from workagent.backend.services.ontology_remediation_kb import get_remediation
