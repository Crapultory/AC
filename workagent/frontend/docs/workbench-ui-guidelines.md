...
This guide keeps WORKAGENT workbench pages visually consistent while preserving current behavior contracts.
...
- Color/surface tokens:
  - `--workagent-bg`, `--workagent-bg-alt`
  - `--workagent-panel`, `--workagent-panel-strong`, `--workagent-surface`
  - `--workagent-accent`, `--workagent-accent-strong`, `--workagent-accent-soft`
  - `--workagent-text`, `--workagent-muted`, `--workagent-danger`
  - `--workagent-border`, `--workagent-border-strong`
- Shape and motion tokens:
  - `--workagent-radius-sm`, `--workagent-radius-md`, `--workagent-radius-lg`
  - `--workagent-shadow`
  - `--workagent-transition`
- Layer tokens:
  - Canonical: `--workagent-z-content`, `--workagent-z-sticky`, `--workagent-z-drawer`, `--workagent-z-modal`, `--workagent-z-modal-critical`
  - Compatibility aliases: `--z-content`, `--z-sticky`, `--z-drawer`, `--z-modal`, `--z-modal-critical`

Rules:

1. Prefer `--workagent-*` tokens directly for new code.
2. Avoid introducing page-specific color/radius constants unless a token gap is real.
3. Keep typography/body contrast aligned with `--workagent-text` and `--workagent-muted`.
...
- `--workagent-z-content` (`10`): default cards, panes, and page content
- `--workagent-z-sticky` (`20`): sticky local controls/bars that should sit above content
- `--workagent-z-drawer` (`30`): drawers or side overlays
- `--workagent-z-modal` (`50`): standard modal dialogs
- `--workagent-z-modal-critical` (`60`): confirmation/critical dialogs that must top regular modals
