...
4. Keep the artifact self-contained except for permitted CDN fonts, icons, or chart libraries. Include the essential text and data in the DOM so the document remains useful if a CDN fails or JavaScript is disabled. Escape untrusted text and do not embed secrets.
...
1. Copy the complete contents of `assets/agent2ui-bridge.js` into an inline `<script>` immediately before `</body>`. This resource is the only bridge source; never reference it with `<script src>`, omit it, or add a second bridge implementation. The delivered HTML must remain a standalone file.
...
The Aegis preview iframe permits scripts and forms, but remains cross-origin, navigation-restricted, and popup-restricted. Use forms only for local interactive controls whose results are handled in the document and delivered with the Agent2UI bridge.
