...
export default function OverviewTab({
  ...
  return (
    <div className="flex-1 space-y-4 overflow-y-auto p-4 scrollbar-thin">
      <div className="flex flex-col items-start justify-between gap-4 rounded-xl border border-slate-800 bg-[#05080F] p-4 shadow-lg md:flex-row md:items-center">
        <div>
          <div className="flex items-center gap-2"><span className="h-2 w-2 animate-pulse rounded-full bg-emerald-500" /><h2 className="flex items-center gap-2 text-sm font-bold uppercase tracking-wider text-white italic">Aegis Center Overview <span className="rounded border border-cyan-800/40 bg-cyan-950/40 px-2 py-0.5 font-mono text-[10px] font-normal text-cyan-400">LIVE</span></h2></div>
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-3">
        <section className="flex min-h-[560px] flex-col overflow-hidden rounded-xl border border-slate-800 bg-[#05080F] xl:col-span-2">
          <div className="z-10 flex flex-col justify-between gap-3 border-b border-slate-800 bg-[#03060C] p-4 sm:flex-row sm:items-center"><div><h3 className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-white italic">Aegis Orchestration Topology <span className="rounded border border-cyan-800 px-1.5 py-0.5 font-mono text-[10px] text-cyan-400">STARMAPPING</span></h3><p className="mt-1 text-[10px] text-slate-500">Hover or focus a node to inspect and illuminate its orchestration path.</p></div><span className="font-mono text-[9px] tracking-wider text-slate-500">API · {topology ? `${topology.agents.length} AGENTS / ${topology.edges.length} EDGES` : 'CONNECTING'}</span></div>
          <StarmappingTopology topology={topology} error={topologyError} />
        </section>

        <aside className="flex flex-col gap-4">
          <div className="relative flex items-center justify-between overflow-hidden rounded-xl border border-slate-800 bg-[#05080F] p-4"><div className="absolute right-0 top-0 h-24 w-24 rounded-full bg-cyan-500/5 blur-2xl" /><div className="mr-2"><div className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Aegis Delegation Health (7D)</div><h4 className="mt-1 text-sm font-bold text-white italic">{successRate === null ? 'No delegations' : 'Delegation success rate'}</h4><p className="mt-1 text-[11px] leading-normal text-slate-400">{stats ? successRate === null ? 'No delegate audits were recorded in this window.' : `${stats.success_count} succeeded of ${stats.delegation_total} delegations.` : metricsUnavailable ? 'Overview metrics are currently unavailable.' : 'Loading delegation metrics...'}</p>{stats && successRate !== null ? <p className="mt-1 font-mono text-[10px] text-cyan-400">{formatRateChange(stats.comparison.success_rate_change_percentage_points)}</p> : null}{stats ? <div className="mt-2 flex flex-wrap gap-1.5 font-mono text-[9px]"><span className="rounded border border-emerald-900/40 px-1.5 py-0.5 text-emerald-400">succ {stats.status_counts.succ}</span><span className="rounded border border-rose-900/40 px-1.5 py-0.5 text-rose-400">fail {stats.status_counts.fail}</span><span className="rounded border border-amber-900/40 px-1.5 py-0.5 text-amber-400">denied {stats.status_counts.auth_denied}</span></div> : null}</div><div className="relative flex shrink-0 items-center justify-center"><svg width="72" height="72" viewBox="0 0 36 36" className="-rotate-90 transform"><circle cx="18" cy="18" r="15.91" fill="none" stroke="#1e293b" strokeWidth="2.5" /><circle cx="18" cy="18" r="15.91" fill="none" stroke="url(#postureGradient)" strokeWidth="2.5" strokeDasharray={`${successRatePercent} 100`} /><defs><linearGradient id="postureGradient" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stopColor="#06b6d4" /><stop offset="100%" stopColor="#10b981" /></linearGradient></defs></svg><span className="absolute font-mono text-sm font-extrabold text-white">{successRate === null ? '—' : formatPercent(successRate)}</span></div></div>

          <div className="flex min-h-[360px] flex-1 flex-col overflow-hidden rounded-xl border border-slate-800 bg-[#05080F]"><div className="space-y-3 border-b border-slate-800 bg-[#03060C] p-4"><div className="flex items-center justify-between"><h4 className="text-[11px] font-bold uppercase tracking-widest text-white italic">Agent Index</h4><span className="rounded border border-slate-800 bg-[#080C14] px-1.5 py-0.5 font-mono text-[9px] font-bold text-cyan-400">Total: {agents.length}</span></div><div className="flex gap-1.5"><div className="relative flex-1"><Search className="absolute left-2.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-slate-500" /><input value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} placeholder="Search agents..." className="w-full rounded border border-slate-800 bg-[#020408] py-1 pl-8 pr-2 font-mono text-xs text-white placeholder-slate-600 focus:border-cyan-500 focus:outline-none" /></div><select value={selectedStatus} onChange={(event) => setSelectedStatus(event.target.value)} className="rounded border border-slate-800 bg-[#020408] px-1.5 py-1 font-mono text-[11px] text-cyan-400 focus:border-cyan-500 focus:outline-none"><option value="All">All Status</option><option value="Active">Active</option><option value="Idle">Idle</option><option value="Offline">Offline</option></select></div></div><div className="max-h-[380px] flex-1 divide-y divide-slate-800/60 overflow-y-auto scrollbar-thin">{filteredAgents.length === 0 ? <div className="p-8 text-center font-mono text-xs text-slate-500">No agents found</div> : filteredAgents.map((agent) => <div key={agent.id} className="flex items-center justify-between border-l-2 border-transparent p-3 text-[11px] transition-all hover:border-cyan-500 hover:bg-[#03060C]"><div className="min-w-0 flex-1 pr-2"><div className="flex items-center gap-1.5"><span className={`h-1.5 w-1.5 rounded-full ${agent.status === 'Active' ? 'bg-emerald-500' : agent.status === 'Idle' ? 'bg-amber-400' : 'bg-gray-600'}`} /><span className="truncate font-bold text-slate-200">{agent.name}</span><span className="rounded bg-slate-800/40 px-1 font-mono text-[8px] uppercase text-slate-500">{agent.type === 'agent' ? 'Co-Agent' : 'VIP'}</span></div><p className="mt-0.5 truncate text-[10px] text-slate-500">{agent.description}</p></div><div className="shrink-0 text-right font-mono text-[9px] text-slate-500"><div className="font-semibold text-cyan-400">Tasks: {agent.tasksCount}</div><div>{agent.lastUpdated}</div></div></div>)}</div><div className="border-t border-slate-800 bg-[#03060C] p-2.5 text-center font-mono text-[9px] text-slate-500">UNIFIED_COORDINATOR_ROUTING: ACTIVE</div></div>
        </aside>
      </div>
      {isAdmin ? <p className="px-1 font-mono text-[9px] text-slate-600">Topology definition is API-backed; runtime status is sourced from the A2A registry.</p> : null}
    </div>
  );
}
