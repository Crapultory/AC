...
<head>
  ...
  <title>AISOC A2A Management</title>
  <style nonce="{nonce}">
...
<main>
  <header>
    <span class="tag">SERVICE / AISOC-A2A</span>
...
(() => {{
  ...
  restartButton.addEventListener("click", async () => {{
    ...
    if (!window.confirm("Danger: restart the AISOC A2A process? Active requests may be interrupted.")) return;
