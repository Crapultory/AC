...
export const DOMAIN_META: Record<string, { color: string; label_en: string; label_zh: string }> = {
  ...
  D3: { color: '#6ee7b7', label_en: 'Investigation & Triage', label_zh: '安全调查与研判' },
  D4: { color: '#31C8E0', label_en: 'Threat Hunting', label_zh: '主动威胁狩猎' },
  D5: { color: '#C08CF7', label_en: 'Threat Intelligence', label_zh: '威胁情报管理' },
  D6: { color: '#fcd34d', label_en: 'Vulnerability & Exposure', label_zh: '漏洞与暴露面管理' },
  D7: { color: '#fb923c', label_en: 'Incident Response', label_zh: '事件响应与自动化处置' },
  D8: { color: '#fda4af', label_en: 'Adversary Emulation', label_zh: '攻击模拟与检测验证' },
  ...
  AST: { color: '#6ee7b7', label_en: 'Asset & Identity', label_zh: '资产与身份' },
  NET: { color: '#31C8E0', label_en: 'Network & Exposure', label_zh: '网络与暴露面' },
  ING: { color: '#4C9BFF', label_en: 'Data Ingestion', label_zh: '数据接入' },
  DET: { color: '#fcd34d', label_en: 'Detection', label_zh: '检测分析' },
  IR: { color: '#fb923c', label_en: 'Incident Response', label_zh: '事件响应' },
...
export const LAYER_META: Record<number, { shape: string; label: string; accent: string }> = {
  ...
  2: { shape: 'round-rectangle', label: '② 二级功能', accent: '#7dd3fc' }, // subcaps: rounded-rect, cyan tint
